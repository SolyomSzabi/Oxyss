...
const AllAppointments = () => {
      ...
      const barbersWithData = await Promise.all(
        allBarbers.map(async (barber) => {
          try {
            ...
            console.error(`Error fetching data for ${barber.name}:`, err);
            return { ...barber, appointments: [], breaks: [] };
          }
        })
      );
  ...
  const getBreakPosition = (startTime, endTime) => {
  ...
  const handleDeleteClick = (appointment) => setDeletingAppointment(appointment);
  ...
  const handleCreateAppointment = async () => {
    ...
    if (!newAppointmentData.customer_name.trim()) { toast.error('Customer name is required'); return; }
    if (!newAppointmentData.customer_phone.trim()) { toast.error('Customer phone is required'); return; }
    if (!newAppointmentData.customer_email.trim()) { toast.error('Customer email is required'); return; }
    if (!newAppointmentData.service_id) { toast.error('Please select a service'); return; }
    ...
    try {
      ...
      if (availableDuration < requestedDuration) {
        ...
        if (actualDuration < 15 && availableDuration >= 15) actualDuration = 15;
  ...
  const handleSaveBreak = async () => {
    if (!creatingBreak) return;
    if (!newBreakData.start_time) { toast.error('Start time is required'); return; }
    if (!newBreakData.end_time) { toast.error('End time is required'); return; }
    if (!newBreakData.title.trim()) { toast.error('Title is required'); return; }

    const [sh, sm] = newBreakData.start_time.split(':').map(Number);
    const [eh, em] = newBreakData.end_time.split(':').map(Number);
  ...
  const handleDeleteBreakClick = (breakItem) => {
    setDeletingBreak(breakItem);
    setDeletingBreakConfirm(true);
  };

  const handleCloseDeleteBreakDialog = () => {
    setDeletingBreak(null);
    setDeletingBreakConfirm(false);
  };

  const handleConfirmDeleteBreak = async () => {
    ...
    return (
      <div className="min-h-screen bg-zinc-50 flex items-center justify-center pt-16">
        <div className="text-center">
          ...
          <Button onClick={fetchAllAppointments} variant="outline">Try Again</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-zinc-50 pt-16">
      ...
      <section className="bg-white border-b sticky top-16 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between mb-3">
            <Button variant="ghost" onClick={() => navigate('/barber-dashboard')} className="flex items-center space-x-2" size="sm">
              <ArrowLeft className="h-4 w-4" />
              <span>Back</span>
            </Button>
            <Button onClick={() => fetchAllAppointments()} variant="outline" size="sm">Refresh</Button>
          </div>
          ...
          <div className="flex items-center space-x-3 mb-3">
            <Input
              ...
              className="w-40"
            />
            <Button variant="outline" size="sm" onClick={() => fetchAllAppointments(selectedDate)}>Load</Button>
          </div>

          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold font-heading text-zinc-900 mb-1">Today's Schedule - All Staff</h1>
            ...
            <div className="text-right">
              <div className="text-2xl font-bold text-yellow-600">{getTotalAppointments()}</div>
              <div className="text-xs text-zinc-600">Total Appointments</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          {/* Legend */}
          <div className="p-4 border-b bg-zinc-50 flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center space-x-6 text-sm flex-wrap gap-y-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-zinc-700">Break</span>
              </div>
              <div className="flex items-center space-x-2">
          ...
          <div className="overflow-x-auto">
            <div className="flex min-w-max">
              ...
              <div className="w-20 flex-shrink-0 border-r bg-zinc-50">
                <div className="h-12 border-b flex items-center justify-center font-semibold text-sm text-zinc-700">Time</div>
                {businessHours.map((hour) => (
                  <div key={hour} className="h-24 border-b flex items-start justify-center pt-2 text-sm text-zinc-600">
                    {hour}:00
                  </div>
                ))}
              </div>
                ...
                <div key={barber.id} className="flex-1 min-w-64 border-r relative">
                  ...
                  <div className="h-12 border-b bg-zinc-900 text-white flex items-center justify-between px-3">
                    <div>
                      ...
                      <div className="text-xs text-gray-300">
                        {barber.appointments.length} appt{barber.appointments.length !== 1 ? 's' : ''}
                  ...
                  <div className="relative" style={{ height: `${businessHours.length * 96}px` }}>
                    ...
                    {currentTimePosition !== null && (
                      <div className="absolute w-full border-t-2 border-red-500 z-20" style={{ top: `${currentTimePosition}%` }}>
                        <div className="absolute -top-2 -left-2 w-4 h-4 bg-red-500 rounded-full" />
                      </div>
                    )}

                    {/* Breaks */}
                    {(barber.breaks || []).map((breakItem) => {
                      const startTime = breakItem.start_time || breakItem.start;
                      const endTime = breakItem.end_time || breakItem.end;
                      if (!startTime || !endTime) return null;
                      const position = getBreakPosition(startTime, endTime);

                      return (
                        <div
                          key={`break-${breakItem.id}`}
                          className="absolute w-full px-0.5 z-10 break-card"
                          style={{ top: position.top, height: position.height, minHeight: '24px' }}
                        >
                          <div className="h-full rounded p-1 shadow-sm border-l-4 border-orange-400 bg-orange-50 border-orange-200 flex items-center justify-between">
                            <div className="flex items-center gap-1 min-w-0">
                              <Coffee className="h-3 w-3 text-orange-500 flex-shrink-0" />
                              <div className="min-w-0">
                                <div className="text-[9px] font-bold text-orange-800 truncate">{breakItem.title || 'Break'}</div>
                                <div className="text-[8px] text-orange-600 truncate">
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDeleteBreakClick(breakItem);
                              }}
                              className="flex-shrink-0 ml-1 bg-orange-100 hover:bg-orange-200 px-1 py-0.5 rounded border border-orange-300 transition-colors"
                              title="Delete break"
                            >
                              <Trash2 className="w-2.5 h-2.5 text-orange-700" />
                            </button>
                          </div>
                        </div>
                      );
                            ...
                            <div className="h-full flex text-[9px] leading-tight">
                              ...
                              <div className="flex flex-col items-end gap-0.5 flex-shrink-0">
                                <div
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    if (appointment.status === 'confirmed') handleEditDuration(appointment);
                                  }}
                    ...
                    {/* Empty State */}
                    {barber.appointments.length === 0 && (barber.breaks || []).length === 0 && (
      ...
      <Dialog open={!!creatingAppointment} onOpenChange={handleCloseCreateDialog}>
        <DialogContent className="sm:max-w-md bg-white" onPointerDownOutside={(e) => e.preventDefault()}>
          <DialogHeader className="space-y-2 pb-4 border-b border-zinc-200">
            ...
            <DialogDescription className="text-sm text-zinc-600">Add a new appointment for the selected time slot.</DialogDescription>
          </DialogHeader>
          {creatingAppointment && (
            <div className="space-y-4 py-4">
              <div className="bg-gradient-to-br from-yellow-50 to-orange-50 p-4 rounded-lg space-y-2 text-sm border border-yellow-200">
                <div className="text-zinc-800"><span className="font-semibold text-zinc-900">Barber:</span> {barbers.find(b => b.id === creatingAppointment.barber_id)?.name}</div>
                <div className="text-zinc-800"><span className="font-semibold text-zinc-900">Date:</span> {formatSelectedDate()}</div>
                <div className="text-zinc-800"><span className="font-semibold text-zinc-900">Time:</span> {formatTime(creatingAppointment.time)}</div>
              </div>
              <div className="space-y-3">
                <div>
                  <label className="text-sm font-semibold text-zinc-900 block mb-2">Customer Name *</label>
                  <Input value={newAppointmentData.customer_name} onChange={(e) => setNewAppointmentData({...newAppointmentData, customer_name: e.target.value})} placeholder="Enter customer name" disabled={creating} />
                </div>
                <div>
                  <label className="text-sm font-semibold text-zinc-900 block mb-2">Customer Phone *</label>
                  <Input value={newAppointmentData.customer_phone} onChange={(e) => setNewAppointmentData({...newAppointmentData, customer_phone: e.target.value})} placeholder="Enter phone number" disabled={creating} />
                </div>
                <div>
                  <label className="text-sm font-semibold text-zinc-900 block mb-2">Customer Email *</label>
                  <Input type="email" value={newAppointmentData.customer_email} onChange={(e) => setNewAppointmentData({...newAppointmentData, customer_email: e.target.value})} placeholder="Enter email address" disabled={creating} />
                </div>
                <div>
                  <label className="text-sm font-semibold text-zinc-900 block mb-2">Service *</label>
                  <Select value={newAppointmentData.service_id} onValueChange={(value) => setNewAppointmentData({...newAppointmentData, service_id: value})} disabled={creating}>
                ...
                <div>
                  <label className="text-sm font-semibold text-zinc-900 block mb-2">Notes (Optional)</label>
                  <Input value={newAppointmentData.notes} onChange={(e) => setNewAppointmentData({...newAppointmentData, notes: e.target.value})} placeholder="Add any notes" disabled={creating} />
                </div>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={handleCloseCreateDialog} disabled={creating}>Cancel</Button>
            <Button onClick={handleCreateAppointment} disabled={creating} className="bg-yellow-600 hover:bg-yellow-700">
              {creating ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Creating...</> : <><Plus className="h-4 w-4 mr-2" />Create Appointment</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {/* ── Add Break Dialog ── */}
      <Dialog open={!!creatingBreak} onOpenChange={handleCloseBreakDialog}>
        <DialogContent className="sm:max-w-md bg-white" onPointerDownOutside={(e) => e.preventDefault()}>
          <DialogHeader className="space-y-2 pb-4 border-b border-zinc-200">
            <DialogTitle className="text-xl font-bold text-zinc-900">Add Break</DialogTitle>
            <DialogDescription className="text-sm text-zinc-600">
              Schedule a break for {creatingBreak ? barbers.find(b => b.id === creatingBreak.barber_id)?.name : ''} on {formatSelectedDate()}.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div>
              <label className="text-sm font-semibold text-zinc-900 block mb-2">Title *</label>
              <Input
                value={newBreakData.title}
                onChange={(e) => setNewBreakData({...newBreakData, title: e.target.value})}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-semibold text-zinc-900 block mb-2">Start Time *</label>
                <Input
                  type="time"
                  value={newBreakData.start_time}
                  onChange={(e) => setNewBreakData({...newBreakData, start_time: e.target.value})}
                  disabled={savingBreak}
                />
              </div>
              <div>
                <label className="text-sm font-semibold text-zinc-900 block mb-2">End Time *</label>
                <Input
                  type="time"
                  value={newBreakData.end_time}
                  onChange={(e) => setNewBreakData({...newBreakData, end_time: e.target.value})}
                  disabled={savingBreak}
                />
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={handleCloseBreakDialog} disabled={savingBreak}>Cancel</Button>
            <Button onClick={handleSaveBreak} disabled={savingBreak} className="bg-orange-500 hover:bg-orange-600">
              {savingBreak ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Saving...</> : <><Coffee className="h-4 w-4 mr-2" />Add Break</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editingAppointment} onOpenChange={handleCloseDurationDialog}>
        <DialogContent className="sm:max-w-md bg-white" onPointerDownOutside={(e) => e.preventDefault()}>
          <DialogHeader className="space-y-2 pb-4 border-b border-zinc-200">
            ...
            <DialogDescription className="text-sm text-zinc-600">Modify the duration for this appointment. You can only reduce the duration, not increase it.</DialogDescription>
          </DialogHeader>
          {editingAppointment && (
            <div className="space-y-4 py-4">
              <div className="bg-gradient-to-br from-yellow-50 to-orange-50 p-4 rounded-lg space-y-2 text-sm border border-yellow-200">
              ...
              <div className="space-y-3 bg-zinc-50 p-4 rounded-lg border border-zinc-200">
                <label className="text-sm font-semibold text-zinc-900 uppercase tracking-wide block">New Duration (minutes)</label>
                ...
                <div className="bg-blue-50 p-3 rounded border border-blue-200">
                  <p className="text-xs text-blue-900 font-medium">📌 Minimum: 15 minutes | Maximum: {editingAppointment.duration} minutes</p>
                </div>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={handleCloseDurationDialog} disabled={updating}>Cancel</Button>
            <Button onClick={handleSaveDuration} disabled={updating} className="bg-yellow-600 hover:bg-yellow-700">
              {updating ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Saving...</> : <><Save className="h-4 w-4 mr-2" />Save Changes</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deletingAppointment} onOpenChange={handleCloseDeleteDialog}>
        <DialogContent className="sm:max-w-md bg-white" onPointerDownOutside={(e) => e.preventDefault()}>
          <DialogHeader className="space-y-2 pb-4 border-b border-zinc-200">
            ...
            <DialogDescription className="text-sm text-zinc-600">Are you sure you want to delete this appointment? This action cannot be undone.</DialogDescription>
          </DialogHeader>
          {deletingAppointment && (
            <div className="space-y-4 py-4">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={handleCloseDeleteDialog} disabled={deleting}>Cancel</Button>
            <Button onClick={handleConfirmDelete} disabled={deleting} className="bg-red-600 hover:bg-red-700 text-white">
              {deleting ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Deleting...</> : <><Trash2 className="h-4 w-4 mr-2" />Delete Appointment</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {/* ── Delete Break Dialog ── */}
      <Dialog open={deletingBreakConfirm} onOpenChange={handleCloseDeleteBreakDialog}>
        <DialogContent className="sm:max-w-md bg-white" onPointerDownOutside={(e) => e.preventDefault()}>
          <DialogHeader className="space-y-2 pb-4 border-b border-zinc-200">
            <DialogTitle className="text-xl font-bold text-zinc-900">Delete Break</DialogTitle>
            <DialogDescription className="text-sm text-zinc-600">Are you sure you want to delete this break? This action cannot be undone.</DialogDescription>
          </DialogHeader>
          {deletingBreak && (
            <div className="space-y-4 py-4">
              <div className="bg-gradient-to-br from-orange-50 to-yellow-50 p-4 rounded-lg space-y-2 text-sm border border-orange-200">
                <div className="text-zinc-800"><span className="font-semibold text-zinc-900">Title:</span> {deletingBreak.title || 'Break'}</div>
                <div className="text-zinc-800"><span className="font-semibold text-zinc-900">Time:</span> {formatTime(deletingBreak.start_time || deletingBreak.start)} – {formatTime(deletingBreak.end_time || deletingBreak.end)}</div>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={handleCloseDeleteBreakDialog} disabled={deleting}>Cancel</Button>
            <Button onClick={handleConfirmDeleteBreak} disabled={deleting} className="bg-red-600 hover:bg-red-700 text-white">
              {deleting ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Deleting...</> : <><Trash2 className="h-4 w-4 mr-2" />Delete Break</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
